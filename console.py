#!/usr/bin/python3
"""
Command interpreter for STACK-CI project
"""
import cmd
from models import storage, CNC


class STACKCommand(cmd.Cmd):
    """
    ...
    def __class_err(self, arg):
        ...
        error = 0
        if len(arg) == 0:
            print(STACKCommand.ERR[0])
            error = 1
        else:
            if isinstance(arg, list):
                arg = arg[0]
            if arg not in CNC.keys():
                print(STACKCommand.ERR[1])
                error = 1
        return error
    ...
    def __handle_update_err(self, arg):
        ...
        for k in storage_objs.keys():
            if arg[1] in k and arg[0] in k:
                key = k
        if len(arg) < 3:
            print(STACKCommand.ERR[4])
        elif len(arg) < 4:
            print(STACKCommand.ERR[5])
        else:
